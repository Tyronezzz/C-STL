...
private:
  ...
  static size_t ROUND_UP(size_t bytes) {               // 调整至8的边界
...
private:
  ...
  static size_t FREELIST_INDEX(size_t bytes) {        //定位至链表节点号
    return (((bytes) + __ALIGN-1)/__ALIGN - 1);
  }

  // Returns an object of size n, and optionally adds to size n free list.
  static void *refill(size_t n);                    //在当前节点下开辟一块新的空间
...
public:
  ...
  static void * allocate(size_t n)  //n must be > 0
  {
    ...
    my_free_list = free_list + FREELIST_INDEX(n);         //定位至链表节点号+链表起始地址 = 当前节点指向的下方free_list地址
    result = *my_free_list;
    if (result == 0) {                              //该节点下空间为空
  ...
  static void deallocate(void *p, size_t n)  //p may not be 0
  {
    ...
    my_free_list = free_list + FREELIST_INDEX(n);          // 此时my_free_list指向第一个free_list block
    q->free_list_link = *my_free_list;                   // q 的next 指向第一个free_list block
    *my_free_list = q;                                  // my_free_list指向 q

    // 这里没有free, 但不是内存泄漏
    // 若p并非从alloc所得，也可并入alloc。但若p所指的大小不是8的倍数，会引发灾难
...
chunk_alloc(size_t size, int& nobjs)
{
  ...
  if (bytes_left >= total_bytes) {        // pool 空间足以满足需求
      result = start_free;
      start_free += total_bytes;
      return(result);
  } else if (bytes_left >= size) {        //只能满足一个及以上
      ...
      if (bytes_left > 0) {
          obj* volatile *my_free_list =                // 碎片应放入哪个链表节点
